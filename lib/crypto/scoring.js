 ...
 * de letras, palabras, n-gramas, vocales, puntuación y patrones improbables.
...
export function scoreSpanish(candidate) {
  ...
  const folded = foldSpanish(normalized);
  ...
  const spanishMarks = graphemes.filter((character) => character === '¿' || character === '¡').length;
  const typographyScore = Math.min(8, spanishMarks * 4);
  ...
  for (const pattern of IMPROBABLE_PATTERNS) structurePenalty += countMatches(normalized, pattern) * 7;
  ...
  const score = frequency + lexical + ngramScore + vowelScore + spaceScore + typographyScore
